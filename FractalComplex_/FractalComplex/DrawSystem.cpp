...
sf::Image DrawSystem::makeImage(Vector2d box, int iterations, bool display) {
	...
	for (int x = 0; x < box.x; x++) {
		for (int y = 0; y < box.y; y++) {
			...
			Color c1 = fromHSV(int((periodH + v * 360 * frequencyH) * amplitudeH) % 360,
				((s+1)%2)/2.0 + 0.5, (s % 2)/2.0 + 0.5);
			//c = fromHSV(int((periodH + v * 360 * frequencyH) * amplitudeH) % 360, 1- (values[x][y].b+ valueConst - 2) * 0.1, (values[x][y].b+ valueConst -2)*0.1);
			if (v < EPS) {
				c = Color(0, 0, 0);
			}
